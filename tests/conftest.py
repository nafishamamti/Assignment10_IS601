...
from app.models.user import User, Base as ModelBase
...
def _resolve_test_engine():
    """
    Build a test engine from TEST_DATABASE_URL.
    If PostgreSQL is unavailable, gracefully fall back to SQLite for local test runs.
    """
    preferred_engine = get_engine(database_url=TEST_DATABASE_URL)
    if TEST_DATABASE_URL.startswith("postgresql"):
        try:
            with preferred_engine.connect() as conn:
                conn.execute(text("SELECT 1"))
            logger.info(f"Using PostgreSQL test database URL: {TEST_DATABASE_URL}")
            return preferred_engine, TEST_DATABASE_URL
        except SQLAlchemyError as exc:
            fallback_url = "sqlite:///./test.db"
            logger.warning(
                "PostgreSQL test database is unavailable (%s). "
                "Falling back to SQLite at %s",
                exc,
                fallback_url,
            )
            fallback_engine = get_engine(database_url=fallback_url)
...
test_engine, ACTIVE_TEST_DATABASE_URL = _resolve_test_engine()
